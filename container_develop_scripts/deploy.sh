...
f_find_manifest () {
  ...
  if [ -n "$manifest" ]
  then
    ( [ -f "$manifest" ] && [ -x "$manifest" ] ) && return 0
    return 1
  fi
...
# TODO вместо pvc.yml надо воткнуть что-то более полезное
  [ -z "$manifest" ] \
    && manifest=$( find . -maxdepth 1 -type f \( -iname "*.yml" -or -iname "*.yaml" \) -not \( -iname "docker-compose.y*ml" -or -iname "pvc.yml" \) -print -quit 2>/dev/null )

  echo "$manifest"
}

# TODO попозже надо увезти это в соответствующие пункты, а то он требуется не всем операциям.
manifest=$(f_find_manifest "$2")
...
'replace'|'rep'|'repl')			# Заменяет манифест
  echo "Используемый манифест: $manifest"
